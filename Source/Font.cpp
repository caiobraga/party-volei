...
SDL_Texture* Font::RenderText(SDL_Renderer* renderer, const std::string& text, const Vector3& color /*= Color::White*/, const Vector3& backgroundColor,
                              int pointSize /*= 24*/, unsigned wrapLength /*= 900*/)
{
    // Convert to SDL_Color
    SDL_Color sdlColor;
    SDL_SetRenderDrawColor(renderer, backgroundColor.x, backgroundColor.y, backgroundColor.z, 255); // Yellow color
    // Swap red and blue so we get RGBA instead of BGRA
    sdlColor.b = static_cast<Uint8>(color.x * 255);
    sdlColor.g = static_cast<Uint8>(color.y * 255);
    sdlColor.r = static_cast<Uint8>(color.z * 255);
    ...
    if (iter != mFontData.end())
    {
        ...
        // Draw this to a surface (blended for alpha)
        SDL_Surface* surf = TTF_RenderUTF8_Blended_Wrapped(font, text.c_str(), sdlColor, wrapLength);
        mTextSurface = TTF_RenderText_Solid(font, text.c_str(), sdlColor);
       // mTextSurface = TTF_RenderUTF8_Blended_Wrapped(font, text.c_str(), sdlColor, wrapLength);
        if (!surf)
        {
            SDL_Log("Failed to create surface for text: %s", text.c_str());
            return nullptr;
